...
var gethwtryLoginData = JSON.parse(localStorage.getItem(gethwtryLoginData_c));
if (gethwtryLoginData === null) {
    localStorage.setItem(gethwtryLoginData_c, "[]");
}
        ...
        success: function success(res) {
            ...
            if (res.code === "1000") {
                ...
                if (!(Array.isArray(ptLoginData) && ptLoginData.length === 0)) {
                    ...
                    for (var its = 0; its < ptLoginData.length; its++) {
                        var tindex = its;
...
// 华为登录
function userToHuaWei() {
    var isFirstHWLogin = localStorage.getItem("FirstHWLogin");
    console.log(isFirstLogin);
    console.log(isClick);

    if (isFirstHWLogin === null) {
        console.log("--------->翠花呀");
        huawei_login()
    } else if (isFirstHWLogin != null && isFirstHWLogin > 0) {
        console.log("现在是直接")
        hwtryLoginData = JSON.parse(localStorage.getItem("hwtryLoginData"));
        console.log(hwtryLoginData);
...
function huawei_login() {
    window.Callback.login("HUAWEI")
}

// 华为登录
function huaweiLogin(token) {
    var uncodeToken = decodeURIComponent(token)
    $(".loading").show();
    $(".showad").show();
    isClick = false;
    if (uncodeToken) {
        $.ajax({
            type: "POST",
            url: Api.userHWLoign_api,
            data: {
                accessToken: uncodeToken,
                packageName: packageName_str,
                language: language_str,
                gameCode: gameCode_str,
                os: os_str,
            ...
            success: function success(res) {
                console.log(res)
                isFirstHWLogin = 1;
                localStorage.setItem("FirstHWLogin", isFirstHWLogin);
                var time = new Date().getTime();
                hwTrydata = {
                    type: "hwLogin",
                    icon: "icon-huawei",
                ...
                hwtryLoginData.push(hwTrydata);
                var obj = {};
                hwtryLoginData = hwtryLoginData.reduce(function (item, next) {

                    obj[next.name] ? '' : obj[next.name] = true && item.push(next);
                    return item;
                }, []);

                localStorage.setItem("hwtryLoginData", JSON.stringify(hwtryLoginData) || "[]");
                if (res.code === "1000") {
                    $.ajax({
                        type: "GET",
                        url: res.url,
...
function huawei_autoLogin() {
    console.log("huawei autoLogin")
    window.Callback.autoLogin("HUAWEI")
}
